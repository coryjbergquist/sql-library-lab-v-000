...
"SELECT characters.name, COUNT(books.name) AS book_count FROM characters LEFT JOIN character_books ON characters.id = character_books.character_id ORDER BY book_count"
end
