...
"SELECT characters.name, COUNT(character_books.book_id) AS book_count FROM characters LEFT JOIN character_books ON characters.id = character_books.character_id GROUP BY characters.name ORDER BY book_count"
end
